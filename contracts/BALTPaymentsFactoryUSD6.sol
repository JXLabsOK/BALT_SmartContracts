...
import "./BALTPaymentVaultUSD6.sol";

interface IBALTPaymentsSubscriptionRegistryUSD6 {
    function isActive(address account) external view returns (bool);
}

interface IERC20MetadataUSD6Factory {
    function decimals() external view returns (uint8);
}

contract BALTPaymentsFactoryUSD6 {
    ...
    constructor(address _subscriptionRegistry) {
        ...
        require(_subscriptionRegistry.code.length > 0, "Registry must be contract");
    ...
    function createPaymentVault(address paymentToken, uint inactivityPeriod) external returns (address) {
        require(
            IBALTPaymentsSubscriptionRegistryUSD6(subscriptionRegistry).isActive(msg.sender),
            "Subscription inactive"
        );

        require(paymentToken != address(0), "Invalid payment token");
        require(paymentToken.code.length > 0, "Payment token must be contract");
        require(IERC20MetadataUSD6Factory(paymentToken).decimals() == 6, "Token must have 6 decimals");
        require(inactivityPeriod > 0, "Invalid inactivity period");

        // msg.sender can be an EOA or a Safe/multisig.
        BALTPaymentVaultUSD6 vault = new BALTPaymentVaultUSD6(
            msg.sender,
            paymentToken,
            inactivityPeriod,
