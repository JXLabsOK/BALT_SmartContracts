...
import "./ERC20InheritanceVaultUSD6V2.sol";

contract ERC20InheritanceFactoryUSD6V2 {
    address public immutable commissionWallet;
    address public immutable token;        // ERC-20 (USDT/USDC) used by this factory
    uint16  public immutable feeBps;       // e.g. 50 (0.5%)
    uint256 public immutable minDeposit;   // minimum net deposit (token units, 6 decimals)
    ...
    function createInheritanceVault(uint256 inactivityPeriod) external returns (address) {
        ...
        ERC20InheritanceVaultUSD6V2 vault = new ERC20InheritanceVaultUSD6V2(
            msg.sender,
            inactivityPeriod,
            commissionWallet,
            token,
