// test/BALTPaymentVaultUSD6.test.js
...
describe("BALTPaymentVaultUSD6", function () {
  let admin, payer, recipient1, recipient2, recipient3, other;
  let registry, token, otherToken, token18, vault;
  ...
  const payerInitialBalance = parseUnits("100000", 6);

  const amount1 = parseUnits("100", 6);
  const amount2 = parseUnits("250", 6);
  const amount3 = parseUnits("50", 6);
  ...
  async function deployVault(
    customToken = token.target,
    customRegistry = registry.target,
    period = shortInactivity
  ) {
  ...
  beforeEach(async () => {
    ...
    token = await MockUSD6
      .connect(admin)
      .deploy("Mock USDC", "mUSDC", initialSupply);
    await token.waitForDeployment();

    otherToken = await MockUSD6
      .connect(admin)
      .deploy("Mock USDT", "mUSDT", initialSupply);
    await otherToken.waitForDeployment();

    const Mock18 = await ethers.getContractFactory("MockERC20");
    token18 = await Mock18.connect(admin).deploy();
    ...
    await (
      await token
        .connect(admin)
        .transfer(payer.address, payerInitialBalance)
    ).wait();
  ...
  it("should register a payment and pull the exact USD6 amount", async function () {
    ...
    expect(await vault.totalRecovered()).to.equal(0);
  ...
  it("should allow payer to cancel before payment is claimable", async function () {
    ...
    expect(await vault.pendingAmount()).to.equal(0);
    expect(await token.balanceOf(vault.target)).to.equal(0);
  });
  ...
  it("should allow vault reuse after Closed when subscription is renewed", async function () {
    await (await approveAndRegisterPayment()).wait();
  ...
  it("should recover excess payment token without touching pending funds", async function () {
    ...
    const excess = parseUnits("10", 6);

    await (
      await token
        .connect(admin)
        .transfer(vault.target, excess)
    ).wait();
  ...
  it("should recover unrelated USD6 tokens sent by mistake", async function () {
    const mistakenAmount = parseUnits("25", 6);

    await (
      await otherToken
        .connect(admin)
        .transfer(vault.target, mistakenAmount)
    ).wait();
